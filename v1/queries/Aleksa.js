...
db.glassdoor.aggregate([
    ...
    {$group: {_id: { industry: "$industry" , industryId: "$industryId"}, "number" : {$sum : "$nubmerOfRatings"}}},
    {$project: {industry : "$_id.industry" , number: "$number" , location: "$location"}},
    {$match: {"number" : {$gt : 0}}},
    {$sort: {"number" : -1}},
...
db.glassdoor.aggregate([
    ...
    {$group : {_id: {name : "$empName" , empId : "$empId" , empSize : "$empSize"} , "num" : {$sum : "$nubmerOfRatings"}, "max" :{ $max : {$sum : "$nubmerOfRatings"}}}},
    {$project: {emp : "$_id.name" , num : "$num", max:"$max"}},
    {$match: {num : {$gt : 0}}},
    {$sort : {"num" : -1}},
    {$limit: 1},
...
db.glassdoor.aggregate([
    {$match : { $and :[ {"benefits.comment":{$regex: "Sport"}},{"benefits.comment":{$regex : "sport"}},{"benefits.comment":{$regex: "sports"}} ,{"benefits.comment":{$regex: "Sports"}} ]}},
